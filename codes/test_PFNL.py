...
import models.archs.PFNL_arch as PFNL_arch
...
def main():
    ...
    if data_mode == 'Vid4':
        if stage == 1:
            model_path = '../experiments/pretrained_models/PFNL_4e4.pth'
        else:
    ...
    if stage == 2:
        HR_in = True
        back_RBs = 20
    model = PFNL_arch.PFNL()

    #### dataset
    if data_mode == 'Vid4':
        test_dataset_folder = '/mnt/data/zhoubo/dataset/Vid4/BIx4'
        GT_dataset_folder = '/mnt/data/zhoubo/dataset/Vid4/GT'
        test_dataset_folder = '/mnt/data/zhoubo/dataset/old'
        GT_dataset_folder = '/mnt/data/zhoubo/videodataset/PIC/TestGT'
    else:
        if stage == 1:
    ...
    save_folder = '../results/{}'.format('other')
    ...
    # for each subfolder
    for subfolder in zip(subfolder_l):
    ...
    logger.info('Save images: {}'.format(save_imgs))
    logger.info('Flip test: {}'.format(flip_test))
    logger.info('Total {} clips. '.format(len(subfolder_l)))
