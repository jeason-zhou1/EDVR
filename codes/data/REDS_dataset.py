...
class REDSDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.opt = opt
        self.transindex = np.load(opt['transfile'],allow_pickle=True)
        ...
        # print('=====================numbers',len(self.paths_GT),self.paths_GT[0])
    ...
    def __getitem__(self, index):
        ...
        names = key.split('_')
        names_a = names[:-1]
        name_a = names_a[0]+'_'+names[1]+'_l' 
        name_b = names[-1]
        center_frame_idx = int(name_b)
        # print('index:',index,key,int(names[1]))
        ...
        if self.opt['border_mode']:
            ...
            if center_frame_idx + interval * (N_frames - 1) > 99:# 训练时的逻辑很简单，没过边界就正向，否则反向
                direction = 0
            ...
            each_trans = self.transindex[int(names[1])]
            center_frame_idx = frame_num(each_trans,center_frame_idx,self.half_N_frames)
            # print('中心位置帧：',int(names[1]),each_trans,center_frame_idx)
            # get the neighbor list
            neighbor_list = list(
        ...
        if self.data_type == 'mc':
            ...
            img_GT = util.read_img(self.GT_env, key, (3, 1080, 1920))
        else:
        ...
        for v in neighbor_list:
            ...
            if self.data_type == 'mc':
                ...
                # print(self.LQ_env, '{}_{:04d}'.format(name_a, v))
        ...
        if self.opt['phase'] == 'train':
            ...
            rlt = util.augment(img_LQ_l, self.opt['use_flip'], self.opt['use_rot'])# 数据增强
            img_LQ_l = rlt[0:-1]
            img_GT = rlt[-1]
