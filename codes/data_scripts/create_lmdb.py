...
def main():
    dataset = 'YK'  # vimeo90K | REDS | general (e.g., DIV2K, 291) | DIV2K_demo |test
...
def REDS(mode):
    ...
    BATCH = 5000  # After BATCH images, lmdb commits, if read_all_imgs = False
    if mode == 'train_sharp':
        img_folder = '../../../../dataset/REDS/train_sharp'
        lmdb_save_path = '../../../../dataset/REDS/train_sharp_wval.lmdb'
        H_dst, W_dst = 720, 1280
    elif mode == 'train_sharp_bicubic':
        img_folder = '../../../../dataset/REDS/train_sharp_bicubic'
        lmdb_save_path = '../../../../dataset/REDS/train_sharp_bicubic_wval.lmdb'
        H_dst, W_dst = 180, 320
    ...
    #### read all the image paths to a list，获取所有文件夹中的所有图片的绝对路径
    print('Reading image path list ...')
    ...
    if read_all_imgs:# 内存足够大可以放下的情况
        ...
        def mycallback(arg):
            '''get the image data and update pbar'''
            key = arg[0]
            dataset[key] = arg[1]# 存储的是读取之后的数据流
            pbar.update('Reading {}'.format(key))

        pool = Pool(n_thread)
        for path, key in zip(all_img_list, keys):
            pool.apply_async(read_image_worker, args=(path, key), callback=mycallback)# 读取所有的图片并且加上对应键值
        pool.close()
        pool.join()
        print('Finish reading {} images.\nWrite lmdb...'.format(len(all_img_list)))
...
def YK(mode):
    ...
    BATCH = 5000  # After BATCH images, lmdb commits, if read_all_imgs = False
    if mode == 'GT':
        img_folder = '../../../../videodataset/PIC/GT'
        lmdb_save_path = '../../../../videodataset/PIC/GT.lmdb'
        H_dst, W_dst = 1080, 1920
    elif mode == 'LR':
        img_folder = '../../../../videodataset/PIC/LR'
        lmdb_save_path = '../../../../videodataset/PIC/LR.lmdb'
        H_dst, W_dst = 270, 480
    elif mode == 'train_blur_bicubic':
        img_folder = '../../../../datasets/REDS/train_blur_bicubic'
        lmdb_save_path = '../../../../datasets/REDS/train_blur_bicubic_wval.lmdb'
    ...
    for idx, (path, key) in enumerate(zip(all_img_list, keys)):
        ...
        if 'flow' in mode:
            ...
            H, W, C = data.shape
            assert H == H_dst and W == W_dst and C == 3, 'different shape.{},{},'.format(W,H)+path
        txn.put(key_byte, data)
        if not read_all_imgs and idx % BATCH == 0:
            txn.commit()
            txn = env.begin(write=True)
