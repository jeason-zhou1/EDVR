...
from models.zloss import CharbonnierLoss,GANLoss
        ...
        if self.is_train:
            ...
            for loss in losses.split('+'):
                ...
                if loss_type == 'l1':
                    ...
                    self.cri_pix = vgg.VGG(loss_type[3:],255).to(self.device)
                elif loss_type.find('GAN')>=0:
                    self.cri_pix = Adversarial(opt['datasets']['train']['GT_size'],loss_type).to(self.device)
                else:
    ...
    def optimize_parameters(self, step):
        ...
        for i, l in enumerate(self.loss):
            if l['function'] is not None:
                ...
                log += l['type']+':'+str(self.loss[i - 1]['function'].loss)+';'

        loss_sum = sum(losses)
        if len(self.loss) > 1:
            log += 'total:'+str(loss_sum.item())

        # set log
        self.log_dict['l_pix'] = log
